...
| SPZ (Niantic) | ≤ 20 | 20 bytes before gzip, which shrinks it further depending on the scene |
  ...
  SPZ ([nianticlabs/spz](https://github.com/nianticlabs/spz)) is written as version 3, the last gzip one (readers of version 4 still read it): 24-bit fixed-point positions, 8-bit opacity, colour and log-scale, and a 32-bit smallest-three rotation, stored attribute by attribute so gzip can squeeze them. It also drops normals and PBR. SPZ is defined in RUB axes and the tools that convert a 3DGS `.ply` to `.spz` flip y and z on the way; the export does the same, so the `.spz` and `.ply` of the same splats show the same way up in a viewer. Log scales are clamped to [-10, 6], so the thin axis of a flat splat comes out at e^-10 rather than ~0. `.spz` files (versions 2 and 3) load back in like a `.ply`.
